...
try {
    $db_conn = new PDO("mysql:host=localhost;port=3306;dbname=classtodos", 'root', 'Rush2112');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($task_name) && !empty($task_date) && !empty($task_time)) {
        ...
        try {
            $sql = "INSERT INTO tasks (task_name, task_date, task_time, status) VALUES (:task_name, :task_date, :task_time, 'pending')";
            ...
            // Redirect back to profile.php (or another page)
...
<body>
    ...
    <div class="container">
        ...
        <!-- Task input form -->
        <form id="task-form" action="profile.php" method="POST">
            <input type="text" id="task-input" name="task_name" placeholder="Add a new task..." required>
            <select id="day-select" name="task_date"></select>
